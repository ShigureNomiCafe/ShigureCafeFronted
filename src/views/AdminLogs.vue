  ...
  <div class="h-screen bg-gray-50 flex flex-col overflow-hidden">
    ...
    <main class="flex-1 flex flex-col overflow-hidden py-4 sm:py-6">
      <AdminPageHeader :title="t('admin-logs.title')" v-model="logStore.filters.search" :loading="logStore.loading"
        ...
        class="relative z-30 shrink-0 mb-4 sm:mb-6">
        <template #extra>
          <BaseSelect v-model="logStore.filters.level" :options="levelOptions"
            class="flex-1 min-w-[140px] md:min-w-[150px] md:flex-none animate-slide-up animate-delay-50"
            @change="handleFilterChange" />
          <BaseSelect v-model="logStore.filters.source" :options="sourceOptions"
            class="flex-1 min-w-[140px] md:min-w-[150px] md:flex-none animate-slide-up animate-delay-50"
            @change="handleFilterChange" />
        </template>
      </AdminPageHeader>
      ...
      <div class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 overflow-hidden pb-4 sm:pb-6">
        <BaseCard body-class="p-0 flex flex-col h-full overflow-hidden"
          class="h-full border-none shadow-sm ring-1 ring-gray-100 animate-slide-up animate-delay-100">
          <!-- Terminal Container -->
          <div class="flex-1 flex flex-col bg-white overflow-hidden relative">
            ...
            <div
              class="shrink-0 px-4 py-2.5 bg-gray-50 flex items-center justify-between border-b border-gray-100 z-20">
              <div class="flex items-center space-x-3">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ t('admin-logs.title')
                  }}</span>
              ...
              <div class="flex items-center space-x-4">
                <button @click="toggleAutoScroll"
                  ...
                  <span>{{ autoScroll ? t('admin-logs.auto-scroll-on') : t('admin-logs.auto-scroll-off') }}</span>
                </button>
              </div>
            </div>
            ...
            <div class="flex-1 relative overflow-hidden flex flex-col">
              ...
              <div ref="logContainer"
                class="flex-1 overflow-y-auto overflow-x-auto scrollbar-thin font-mono text-sm selection:bg-indigo-100 selection:text-indigo-900"
                @scroll="handleManualScroll">
                <div class="min-w-full inline-block align-middle">
                  <!-- Older logs loading indicator -->
                  <div v-if="logStore.loadingOlder"
                    class="py-4 flex justify-center items-center border-b border-gray-50 bg-gray-50/30">
                  ...
                  <div v-for="log in logStore.logs" :key="log.id"
                    class="group flex items-start space-x-3 px-4 py-1.5 hover:bg-indigo-50/30 transition-colors border-b border-gray-50/50 last:border-0 relative w-full">
                    <span class="text-gray-400 shrink-0 select-none font-medium text-[12px] pt-0.5">
                    ...
                    <div class="shrink-0 flex items-center mt-0.5">
                      <span
                        class="px-2 py-0.5 rounded-md bg-gray-100 text-gray-800 font-mono text-[12px] font-bold tracking-tight">
                        {{ log.source }}
                      </span>
                    </div>
                    <span class="text-gray-700 whitespace-pre shrink-0 leading-relaxed text-[12px] pr-4">
                      {{ log.content }}
                    </span>
                  </div>

                  <!-- Empty State within Terminal -->
                  <div v-if="logStore.logs.length === 0"
              ...
              <transition name="fade">
                <button v-if="showScrollBottomBtn" @click="scrollToBottom('smooth')"
                  ...
                  <span
                    class="absolute right-full mr-3 px-2 py-1 bg-gray-900 text-white text-[10px] rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{{ t('admin-logs.scroll-to-bottom') }}</span>
                </button>
              </transition>
            </div>
          </div>
