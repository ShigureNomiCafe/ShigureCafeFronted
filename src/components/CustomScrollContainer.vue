<template>
  <div class="relative group/scroll w-full h-full flex flex-col" @mouseenter="showScrollbar = true"
    @mouseleave="isDragging ? null : (showScrollbar = false)">
    <!-- Content Container -->
    <div ref="containerRef" class="w-full flex-1 overflow-x-auto scrollbar-none pb-4"
    ...
    <div v-if="hasScroll"
      ...
      <div class="absolute top-0.5 h-1.5 rounded-full transition-colors cursor-pointer" :class="[
        isDragging ? 'bg-indigo-500/80' : 'bg-gray-400/80 hover:bg-indigo-500/80'
      ]" :style="{
          width: thumbWidth + 'px',
          transform: `translateX(${thumbOffset}px)`
        }" @mousedown.stop.prevent="startDrag"></div>
    </div>
  </div>
</template>
